        ...
        funnels.find(funnel => funnel.steps.find(step => step.slug && step.slug !== '' ? `/${step.slug}` === pathname : false)) || services.find(service => service.steps.find(step => step.slug && step.slug !== '' ? `/${step.slug}` === pathname : false)) || pathname.includes('/llamadas')
          ...
          : (
            <Navbar design={design} storeData={storeData} politics={politics} style={style}>
              ...
              { children }
              <div className='bottom-4 right-4 flex flex-col gap-2 fixed w-14' style={{ height: design.chatView ? design.instagram ? design.whatsapp ? '180px' : '120px' : design.whatsapp ? '120px' : '60px' : design.instagram ? design.whatsapp ? '120px' : '60px' : '0px' }}>
                {
                ...
                }
                {
                  design.instagram && storeData.instagram && storeData.instagram !== ''
                    ? (
                      <button area-label='Visita nuestro Instagram' id='instagram' className={`${viewInstagram ? 'opacity-1' : 'opacity-0 translate-y-6'} transition-all duration-500 z-50 w-14 h-14 flex bg-purple-600 rounded-full`} onClick={() => window.open(storeData.instagram)}>
