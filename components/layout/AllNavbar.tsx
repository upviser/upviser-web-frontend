            ...
            <Navbar design={design} storeData={storeData} politics={politics} style={style}>
              ...
              { children }
              {
                design.whatsapp && storeData?.phone !== ''
                  ? (
                    <button area-label='Chatea con nosotros por Whatsapp' id='whatsapp' className={`${viewWhatsapp ? 'opacity-1' : 'opacity-0 translate-y-6'} transition-all duration-500 fixed bottom-20 right-4 z-50 w-14 h-14 flex bg-[#25D366] rounded-full`} onClick={() => window.open(`https://wa.me/+56${storeData.phone}`, '_blank')}>
