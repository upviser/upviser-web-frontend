                ...
                <div className='flex gap-6 justify-around flex-wrap'>
                  {
                    services?.find(service => service._id === content.service?.service)?.plans?.plans.map((plan, index) => (
                      <div className={`${index === 0 ? plan1Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 1 ? plan2Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 2 ? plan3Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 3 ? plan4Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : index === 4 ? plan5Loaded ? 'opacity-1' : 'opacity-0 translate-y-6' : ''} transition-all duration-500 p-6 flex flex-col gap-4 w-full max-w-96 justify-between`} key={plan._id} style={{ boxShadow: style.design === 'Sombreado' ? `0px 3px 20px 3px ${style.borderColor}10` : '', borderRadius: style.form === 'Redondeadas' ? `${style.borderBlock}px` : '', border: style.design === 'Borde' ? `1px solid ${style.borderColor}` : '' }}>
